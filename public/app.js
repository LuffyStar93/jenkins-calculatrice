...
function parserExpression(expression) {
    ...
    // Vérifier que l'expression ne contient que des chiffres, des +, des ., des espaces et des -
    if (!/^[\d\s\+\.\-]+$/.test(expression)) {
        throw new Error('Format invalide. Utilisez seulement des chiffres et des +');
    }
    
    // Vérifier qu'il y a au moins un signe +
    if (!expression.includes('+')) {
        throw new Error('Format invalide. Utilisez au moins un signe +');
    }
    
    // Séparer les nombres par le signe +
    const nombres = expression.split('+').map(part => {
        const nombre = parseFloat(part.trim());
        if (isNaN(nombre)) {
            throw new Error(`"${part}" n'est pas un nombre valide`);
        }
        return nombre;
    });
...
async function calculer() {
    ...
    try {
        ...
        // Appel à l'API - maintenant on envoie l'expression complète
        const response = await fetch('/api/addition', {
            ...
            body: JSON.stringify({
                expression: expression // On envoie l'expression complète
            })
        });
