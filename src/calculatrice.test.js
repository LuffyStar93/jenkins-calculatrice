...
describe('Module Calculatrice', () => {
  describe('addition()', () => {
    ...
    // Tests avec des chaînes de caractères (mode expression)
    test('devrait convertir et additionner des chaînes numériques', () => {
      // Avec ton code, une string est traitée comme UNE expression.
      expect(addition('5+3')).toBe(8);
    ...
    test('devrait lever une erreur pour des entrées non numériques', () => {
      // En mode "expression" (string), ton code lève `"xxx" n'est pas un nombre valide`
      expect(() => addition('abc+5')).toThrow(/n'est pas un nombre valide/);
      expect(() => addition('5+xyz')).toThrow(/n'est pas un nombre valide/);
      expect(() => addition('hello+world')).toThrow(/n'est pas un nombre valide/);
    });
    
    test('devrait lever une erreur pour des valeurs non définies', () => {
      // Ici on déclenche le mode "2 arguments" (pas string) => message générique
    ...
    // Tests avec des espaces (toujours en mode expression)
